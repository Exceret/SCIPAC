...
```{r,eval=FALSE}
sc.meta <- readRDS(system.file(
  "extdata",
  "sim.raw.scRNA.meta.rds",
  package = "SCIPAC"
))
...
plot.dat$log.pval.adj <- ifelse(
  (plot.dat$sig == "Not.sig"),
  0,
  plot.dat$log.pval
)
```
...
plot.dat$phe <- factor(
  plot.dat$phe,
  levels = c("Cell type 1", "Cell type 2", "Cell type 3")
)
...
p3 <- ggplot() +
  geom_point(
    data = plot.dat.negative,
    aes(x = umap.X1, y = umap.X2, col = pval.adj.adj)
  ) +
  scale_color_gradient2(
    '   ',
    low = "darkblue",
...
sc.dat.ordinal <- readRDS(system.file(
  "extdata",
  "sc.dat.ordinal.rds",
  package = "SCIPAC"
))
# Load the meta data for scRNA-seq data
sc.meta.ordinal <- readRDS(system.file(
  "extdata",
  "sim.raw.scRNA.meta.ordinal.rds",
  package = "SCIPAC"
))
# Load the pre-processed and dimension-reduced bulk RNA-seq data
bulk.dat.ordinal <- readRDS(system.file(
  "extdata",
  "bulk.dat.ordinal.rds",
  package = "SCIPAC"
))
# Load the meta data for bulk RNA-seq data
bulk.meta.ordinal <- readRDS(system.file(
  "extdata",
  "bulkRNA.meta.ordinal.rds",
  package = "SCIPAC"
))
```
...
```{r,eval=FALSE}
SCIPAC.res.ordinal <- SCIPAC(
  bulk.dat = bulk.dat.ordinal,
  y = y,
  family = "cumulative",
  ct.res = ct.res.ordinal,
  ela.net.alpha = 0.4,
  bt.size = 50,
  numCores = 7,
  CI.alpha = 0.05,
...
sc.pca.ordinal <- readRDS(system.file(
  "extdata",
  "sc.pca.ordinal.rds",
  package = "SCIPAC"
))
...
```{r,eval=FALSE}
# Plot the paths
colors.cell <- tableau_color_pal("Classic 20", direction = 1)(length(unique(
  plot.dat$Group
)))
p1 <- ggplot(plot.dat) +
...
p3 <- ggplot(plot.dat) +
  ...
  theme(
    legend.title = element_text(family = "sans", size = 27),
    legend.position = "top",
    axis.text.x = element_blank(),
    axis.text.y = element_blank(),
    panel.background = element_blank(),
...
p4 <- ggplot() +
  geom_point(
    data = plot.dat.positive,
    aes(x = PC1, y = PC2, col = pval.adj.adj)
  ) +
  scale_color_gradient2(
    "p-values",
    low = "red",
...
LUAD.sc.dat <- readRDS(system.file(
  "extdata",
  "LUAD.sc.dat.rds",
  package = "SCIPAC"
))
# Load the meta data for LUAD scRNA-seq data
LUAD.sc.meta <- readRDS(system.file(
  "extdata",
  "LUAD.meta.rds",
  package = "SCIPAC"
))
...
LUAD.bulk.dat <- readRDS(system.file(
  "extdata",
  "LUAD.bulk.dat.rds",
  package = "SCIPAC"
))
# Load the meta data for LUAD bulk RNA-seq data
LUAD.bulk.meta <- readRDS(system.file(
  "extdata",
  "LUAD.bulk.meta.rds",
  package = "SCIPAC"
)) %>%
...
```{r,eval=FALSE}
LUAD.bulk.meta$S_time <- ifelse(
  is.na(LUAD.bulk.meta$days_to_death),
...
```{r,eval=FALSE}
SCIPAC.res.LUAD <- SCIPAC(
  bulk.dat = LUAD.bulk.dat.sub,
  y = y,
  family = "cox",
  ct.res = LUAD.ct.res,
  ela.net.alpha = 0.4,
...
```{r,eval=FALSE}
LUAD.sc.umap <- readRDS(system.file(
  "extdata",
  "LUAD.sc.umap.rds",
  package = "SCIPAC"
))
...
plt <- plot.dat %>%
  ...
  guides(
    color = guide_legend(
      override.aes = list(stroke = 1, alpha = 1, shape = 16, size = 4)
    ),
    alpha = FALSE
  ) +
